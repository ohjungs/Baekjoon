...
public class Level7_9 {
	...
	/*
	 * 2941번 크로아티아 알파벳 문제 예전에는 운영체제에서 크로아티아 알파벳을 입력할 수가 없었다. 따라서, 다음과 같이 크로아티아 알파벳을
	 * 변경해서 입력했다.
	 * 
...
/* 아래와 같은 풀이도 가능하다... 
 * Scanner sc = new Scanner(System.in); String croa=sc.nextLine().trim();
 * sc.close(); String[] arr= {"c=","c-","dz=","d-","lj","nj","s=","z="}; int
 * result=0;
 * 
 * for(int i=0;i<arr.length;i++) { if(croa.contains(arr[i])) {
 * croa=croa.replaceAll(arr[i]," "); } } System.out.println(croa.length()); } }
 * 
 */
